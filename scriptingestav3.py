...
# -*- coding: utf-8 -*-
"""
book_fetch.py — Metadatos de libros (ES) con:
...
Uso rápido:
  ...
  # Traducción ON (default). Requiere definir LIBRETRANSLATE_URL
  export LIBRETRANSLATE_URL="https://libretranslate.com/translate"
  python book_fetch.py --title "Words of Radiance" --translate

  # Desactivar traducción
  python book_fetch.py --title "Words of Radiance" --no-translate

  # Profundidad de escaneo de ediciones de Open Library para recolectar subjects (default: 60)
  python book_fetch.py --title "Palabras radiantes" --ol-editions-scan 120
"""
...
def request_json(url: str, params: Optional[dict] = None, timeout: int = 15,
    ...
    try:
        base_headers = {"User-Agent": "book-fetch/1.5"}
        if headers:
...
# Flujo principal
# =========================
def run(isbn: Optional[str], title: Optional[str], author: Optional[str], lang: str = "es",
        translate_flag: bool = True, ol_editions_scan: int = 60) -> None:
    items: List[dict] = []
    prefer_isbn = None
    work_key_hint = None
    ...
    # Cabecera informativa (mantiene formato [INFO], sin warnings ruidosos)
    if work_key:
        ed_count, fpy = openlibrary_work_stats(work_key)
        work_id = work_key.replace("/works/", "")
        info_bits = []
        if ed_count is not None: info_bits.append(f"edition_count={ed_count}")
        if fpy is not None: info_bits.append(f"first_publish_year={fpy}")
...
def main():
    parser = argparse.ArgumentParser(description="Metadatos de libros (ES) con Google Books + Open Library enriquecido + Wikipedia (+traducción).")
    ...
    parser.add_argument("--translate", dest="translate", action="store_true", default=True, help="Traducir sinopsis si está en otro idioma (default: ON)")
    parser.add_argument("--no-translate", dest="translate", action="store_false", help="Desactivar traducción")
    parser.add_argument("--ol-editions-scan", type=int, default=60, help="Cuántas ediciones de Open Library escanear para juntar 'subjects' (default: 60)")
    args = parser.parse_args()

    run(isbn=args.isbn, title=args.title, author=args.author, lang=args.lang,
        translate_flag=args.translate, ol_editions_scan=args.ol_editions_scan)
