...
# -*- coding: utf-8 -*-
"""
FastAPI + persistencia (PostgreSQL) para metadatos de libros:
- Google Books (ediciones, sinopsis, ISBNs)
- Open Library (Work ID + géneros enriquecidos desde subjects del work, ediciones y works candidatos)
- Wikipedia (bio del autor en ES, fallback EN)
- Traducción opcional de sinopsis con LibreTranslate
- Persistencia usando app/db.py (psycopg)

Instalación:
  pip install fastapi uvicorn requests psycopg[binary]
...
except Exception as e:  # fallback si el paquete es plano
    from db import (
        upsert_book,
        attach_genres,
        update_author_description,
...
def request_json(url: str, params: Optional[dict] = None, timeout: int = 15,
    ...
    try:
        base_headers = {"User-Agent": "book-fetch-api/1.1"}
        if headers:
...
def build_output_json(
    ...
    out = {
        ...
        "genres_raw": [],
        "genres_google": genres_google,          # guardamos por si hace falta fallback
...
def merge_with_openlibrary(out_json: Dict[str, Any], work_key: Optional[str],
    ...
    """Subjects de Open Library enriquecidos -> géneros principales. Fallback: categorías de Google."""
    ...
    if all_subjects_raw:
        ...
        out_json["genres"] = dedup(mapped_es + all_subjects_raw)   # mapeados + crudos
...
def aggregate_book_api(isbn: Optional[str], title: Optional[str], author: Optional[str], lang: str = "es",
    ...
    # Limpiar campos auxiliares de Google si no se quieren exponer
...
# =========================
# Persistencia (usa tu app/db.py)
# =========================
def persist_book_record(data: Dict[str, Any], top_genres_n: int = 3) -> Dict[str, Any]:
    """
    Guarda el libro y relaciones básicas:
      - books (isbn, title, author_id por nombre, synopsis)
      - authors.description (bio)
      - book_genres (top N de 'genres')
    """
    # 1) elegir ISBN
    ...
    # autor (el esquema soporta 1 autor -> tomamos el primero si hay varios)
    author_name = None
    author_bio = None
    if data.get("authors"):
    ...
    # 3) upsert libro + autor (por nombre), sin cover por ahora
    upsert_book(
        isbn=isbn,
        title=title_to_use,
        author_name=author_name,
        synopsis=synopsis_to_use,
        cover_obs_key=None,  # lo ven después
    )

    # 4) actualizar descripción del autor si la tenemos
    if author_name and author_bio:
        update_author_description(author_name, author_bio)

    # 5) adjuntar géneros (top N)
    genres = data.get("genres") or []
    attached = attach_genres(isbn, genres, top_n=top_genres_n)
...
# FastAPI app
# =========================
app = FastAPI(title="Book Metadata API", version="1.1.0", description="Google Books + Open Library + Wikipedia (+traducción) con persistencia PostgreSQL")
...
def get_book(
    ...
    persist: bool = Query(False, description="Persistir en BD (usa preferred_isbn13)"),
    top_genres_n: int = Query(3, ge=1, le=20, description="Cantidad de géneros a adjuntar si persist=true"),
):
    ...
    db_result = None
    if persist:
        db_result = persist_book_record(data, top_genres_n=top_genres_n)
...
def save_book(
    payload: Dict[str, Any] = Body(
        ...
        description="Puedes mandar isbn o title (author opcional).",
    )
):
