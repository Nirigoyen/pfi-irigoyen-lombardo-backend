...
DB_URL = "postgresql://root:Livrario123!!!@119.8.155.128:5432/booksdb" #os.getenv("DATABASE_URL")  # p.ej. postgresql://user:pass@host:5432/db
...
def db_search_books_by_title(title: str, limit: int = 10) -> List[Dict[str, Any]]:
    """
    Busca en BD por título (ILIKE). Devuelve lista liviana de matches: [{isbn, title, author}]
    """
    rows: List[Dict[str, Any]] = []
    pattern = f"%{title}%"
    with conn() as c, c.cursor() as cur:
        cur.execute("""
            SELECT b.isbn, b.title, COALESCE(a.name,'') AS author_name
            FROM books b
            LEFT JOIN authors a ON a.id = b.author_id
            WHERE b.title ILIKE %s
            ORDER BY b.updated_at DESC NULLS LAST, b.title
            LIMIT %s
        """, (pattern, limit))
        for isbn, t, author_name in cur.fetchall():
            rows.append({"isbn": isbn, "title": t, "author": author_name})
    return rows

def attach_characters_return(isbn: str, char_names: Iterable[str]) -> List[Dict]:
    """
    Inserta personajes (description NULL) + enlace al libro.
    ...
    with conn() as c, c.cursor() as cur:
        for name in char_names:
            ...
            if not name:
                continue
            # Asegura fila en characters con description NULL
            cur.execute("""
            ...
            cur.execute("""
                ...
                VALUES (%s,%s) ON CONFLICT DO NOTHING
            """, (isbn, cid))
            linked.append({"id": cid, "name": name})
    return linked

def attach_places_return(isbn: str, place_names: Iterable[str], top_n: int = 5) -> List[Dict]:
    """
    Inserta lugares (description NULL) + enlace al libro (con orden).
    Devuelve [{"id": <int>, "name": <str>} ...] (hasta top_n)
    """
    linked = []
    with conn() as c, c.cursor() as cur:
        for ord_idx, name in enumerate(list(place_names)[:top_n], start=1):
            name = (name or "").strip()
            if not name:
                continue
            # Asegura fila en places con description NULL
            cur.execute("""
            ...
            cur.execute("""
                ...
                ON CONFLICT (isbn, place_id) DO NOTHING
            """, (isbn, pid, ord_idx))
            linked.append({"id": pid, "name": name})
    return linked