...
import requests
from fastapi import Body, FastAPI, HTTPException, Query, UploadFile, File, Form, BackgroundTasks
...
try:
    from db import (
        ...
        get_book_info as _get_book_info,  # puede no existir
        attach_characters_return,
        attach_places_return
    )
    HAS_GET_BOOK_INFO = True
except Exception:
    from db import (
        upsert_book,
        attach_genres,
    ...
    _get_book_info = None
    HAS_GET_BOOK_INFO = False

# Intentar importar una función de búsqueda por título si ya la tenés en db.py
try:
    from db import db_search_books_by_title as _db_search_books_by_title  # type: ignore
    HAS_DB_SEARCH_BY_TITLE = True
except Exception:
    _db_search_books_by_title = None
    HAS_DB_SEARCH_BY_TITLE = False

# FallBacks locales si faltan en db.py
def _get_env_conn():
...
def local_get_book_info(isbn: str) -> Optional[dict]:
    """Fallback si db.get_book_info no existe."""
    try:
        import psycopg  # type: ignore
    except Exception:
        raise HTTPException(500, "psycopg no instalado para fallback de get_book_info")

    with _get_env_conn() as c, c.cursor() as cur:
        cur.execute("""
...
def local_db_search_books_by_title(title: str, limit: int = 5) -> List[dict]:
    """Fallback si db.db_search_books_by_title no existe."""
    try:
        import psycopg  # type: ignore
    except Exception:
        raise HTTPException(500, "psycopg no instalado para fallback de búsqueda por título")
    ...
    with _get_env_conn() as c, c.cursor() as cur:
        cur.execute("""
            ...
            ORDER BY b.updated_at DESC NULLS LAST, b.title ASC
            LIMIT %s
        """, (like, limit))
        rows = cur.fetchall()
        return [{"isbn": r[0], "title": r[1], "author": r[2]} for r in rows]
...
def persist_book_record(
    ...
    # NUEVO: obtenemos IDs y nombres enlazados
    chars = data.get("characters") or []
    places = data.get("places") or []
    characters_linked = attach_characters_return(isbn, chars)
    places_linked = attach_places_return(isbn, places, top_n=top_places_n)
    ...
    return {
        ...
        "characters_attached": len(characters_linked),
        "places_attached": len(places_linked),
        "author_description_updated": bool(author_name and author_bio),
        "cover_obs_key_saved": bool(cover_obs_key),
        # devolvemos las listas con ids/nombres para usarlas con la API externa
        "characters_linked": characters_linked,
        "places_linked": places_linked,
    }

# Adaptadores finales usados por la API:
def get_book_info(isbn: str) -> Optional[dict]:
    if HAS_GET_BOOK_INFO and callable(_get_book_info):
        return _get_book_info(isbn)  # type: ignore
    return local_get_book_info(isbn)

def db_search_books_by_title(title: str, limit: int = 5) -> List[dict]:
    if HAS_DB_SEARCH_BY_TITLE and callable(_db_search_books_by_title):
        return _db_search_books_by_title(title, limit)  # type: ignore
...
# =========================
def request_json(url: str, params: Optional[dict] = None, timeout: int = 90,
                 method: str = "GET", data: Optional[dict] = None,
                 headers: Optional[dict] = None) -> Optional[dict]:
    try:
        base_headers = {"User-Agent": "book-fetch-api/1.3", "Accept": "application/json"}
        if headers:
        ...
        if r.status_code == 200:
            return r.json()
        log.warning("HTTP %s %s -> %s body=%s", method, r.url, r.status_code, r.text[:200])
    except requests.RequestException:
        log.exception("request_json error %s %s params=%s", method, url, params)
    return None
...
def fire_and_forget_ingest(pdf_url: str, isbn: str,
                           characters: List[Dict], places: List[Dict]) -> None:
    """
    Dispara la ingesta externa y no bloquea al caller.
    characters/places: lista de objetos {"id": int, "name": str}
    """
    try:
        payload = {
            "pdf_url": pdf_url,
            "isbn": isbn,
            "characters": [{"id": x.get("id"), "name": x.get("name")} for x in (characters or [])],
            "places": [{"id": x.get("id"), "name": x.get("name")} for x in (places or [])],
        }
        requests.post(EXTERNAL_INGEST_URL, json=payload, timeout=5)
    except Exception:
        # Best-effort. No propagamos error.
        pass
...
# =========================
# /book/ingest (multipart) -> PDF a OBS + cover a OBS + persist
# =========================
...
# =========================
# /book/ensure - centraliza: mira BD primero; si no, ingesta completa
# =========================
EXTERNAL_INGEST_URL = os.getenv("EXTERNAL_INGEST_URL", "http://xxxx:yyy/ingest/from_url")
...
async def ensure_book(
    ...
    2) Si no existe -> ingesta completa, persistir y devolver.
       Además, dispara en background la API externa /ingest/from_url con:
       pdf_url, isbn, characters[{id,name}], places[{id,name}]
    """
    if not isbn and not title:
        raise HTTPException(status_code=400, detail="Debes enviar 'isbn' o 'title'.")
    ...
    if isbn:
        try:
            from db import get_book_info, db_search_books_by_title  # asegurar import
        except Exception:
            from db import get_book_info, db_search_books_by_title
    ...
    if (not isbn) and title:
        try:
            from db import db_search_books_by_title
        except Exception:
            from db import db_search_books_by_title
    ...
    # 2.d) Disparar API externa en background (fire-and-forget)
    # Construimos la URL pública del PDF (según convención/OBS)
    pdf_url = f"{OBS_PUBLIC_BASE}/books/{final_isbn}/original.pdf"
    # Tomamos los enlazados con id/name que nos devolvió persist_book_record
    characters_payload = _db.get("characters_linked") or []
    places_payload = _db.get("places_linked") or []
    background_tasks.add_task(
        fire_and_forget_ingest,
        pdf_url=pdf_url,
        isbn=final_isbn,
    ...
    print("===========================| CHARACTERS PAYLOAD |===========================")
    print(characters_payload)
    print("===========================|   PLACES PAYLOAD   |===========================")
    print(places_payload)
