...
async def ensure_book(
    ...
    2) Si no existe -> hacer ingesta completa, persistir y devolver.
    Además, SOLO en la ingesta, se notifica a la API externa con:
      - pdf_url (si se subió el PDF)
      - isbn
      - characters
      - places
    """
    ...
    if not isbn and not title:
        raise HTTPException(status_code=400, detail="Debes enviar 'isbn' o 'title'.")
    if pdf is None:
        print("No se envió PDF (se ingesta solo metadata).")
    else:
